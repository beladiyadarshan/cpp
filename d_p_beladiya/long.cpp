...
def solution(g, a=0, b=0, previous=0, solutions=0, history=0):
    
    if(b==len(g[0])+1):
        return True 
        
    if(previous==0):
        previous=[[True] * (len(g[0])+1) for i in range(len(g)+1)]
        solutions = {}
        history = []
    ...
    ans=0
    i=((a,b), tuple(history[-(len(g)+2):]))
    
    if i in solutions:
        return solutions[i]

    for point in [True, False]:
        if ( not a or not b) or len(set([((previous[a][b-1] + previous[a-1][b] + previous[a-1][b-1] + point)==1), g[a-1][b-1]]))==1:
                history.append(point)
                previous[a][b] = point
                ans+=solution(g, a=(a+1)%(len(g)+1),b=b+(a+1)//(len(g)+1), previous=previous,solutions=solutions, history=history)
                history.pop()
